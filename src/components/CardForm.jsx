...
export default function CardForm({ formData, setFormData, errors, setErrors, onSubmitSuccess }) {
  ...
  const getInputWrapperClass = (errorMsg) => {
    ...
    if (errorMsg) {
      return `${base} bg-red-400`; // 에러 시 빨간색 테두리
    }
    // 포커스 시 보라색 그라디언트 테두리 적용, 기본은 회색 테두리
    return `${base} bg-gray-200 focus-within:bg-gradient-2`;
  };
      ...
      <div className="flex flex-col gap-spacing-100">
        ...
        <div className={getInputWrapperClass(errors.name)}>
          <input
            ...
            className="w-full h-[45px] bg-white px-spacing-200 rounded-[8px] text-preset-3 text-purple-950 placeholder-purple-950/25 border-none outline-none"
          />
        </div>
        {errors.name && (
          <span
            id="name-error"
            role="alert"
            className="text-red-400 text-preset-6 font-medium mt-[2px]"
          >
            {errors.name}
          </span>
        )}
